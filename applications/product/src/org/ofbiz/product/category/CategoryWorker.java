...
public class CategoryWorker {
    ...
    public static List<GenericValue> getRelatedCategoriesRet(Delegator delegator, String attributeName, String parentId, boolean limitView, boolean excludeEmpty, boolean recursive) {
        ...
        try {
        	//通过 产品分类隶属关系表  查询产品   注意ProductCategoryRollup的entity定义
        	//此方法是查询条件为"parentProductCategoryId" = parentId的数据，通过sequenceNum 排序   ，且使用缓存
            ...
            if (limitView) {
                rollups = EntityUtil.filterByDate(rollups, true);//过滤日期
            }
        } catch (GenericEntityException e) {
        ...
        if (rollups != null) {
            ...
            for (GenericValue parent: rollups) {
                ...
                try {
                	//查询title=Current，rel-entity-name=ProductCategory的关联对象
                    cv = parent.getRelatedOne("CurrentProductCategory", true);//查询对应的产品目录数据
                } catch (GenericEntityException e) {
                    Debug.logWarning(e.getMessage(), module);
                }
                if (cv != null) {
                    if (excludeEmpty) {//是否排除空产品
